...
visible simultaneously) but we think the performance benefits vastly outweigh
these.
...
Electron, being Chromium based, uses a multi-process architecture where each
...
processing bursts of text, like running `cat` on a large file (see upper profile
above).
